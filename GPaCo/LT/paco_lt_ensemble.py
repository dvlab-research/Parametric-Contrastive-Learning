...
# emsemble
parser.add_argument('--ensemble', action='store_true', help='evaluate ensemble model')
parser.add_argument('--ensemble_archs', type=str, default=None, help='evaluate ensemble model')
parser.add_argument('--ensemble_paths', type=str, default=None, help='evaluate ensemble model')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print("=> creating ensemble models '{}'".format(args.ensemble_archs))
    ...
    if not args.ensemble:
        ...
        archs = args.ensemble_archs.split(',')
        ensemble_models = []
        for arch in archs:
            arch = arch.strip()
            ensemble_models.append(moco.builder.MoCo(
                models.__dict__[arch] if arch != 'resnext101_32x4d' else getattr(resnet_imagenet, arch),
                args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.mlp, args.feat_dim, num_classes=args.num_classes))
    ...
    if args.distributed:
        ...
        if args.gpu is not None:
            ...
            if not args.ensemble:
               ...
               ensemble_models = [model.cuda(args.gpu) for model in ensemble_models]
            ...
            if not args.ensemble:
                ...
                ensemble_models =[torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu], find_unused_parameters=True) for model in ensemble_models]
        else:
            model.cuda()
    ...
    if args.ensemble:
        ...
        for idx, path in enumerate(checkpoint_paths):
            ...
            checkpoint = torch.load(path.strip(), map_location=loc)
            ensemble_models[idx].load_state_dict(checkpoint['state_dict'])
    ...
    if args.evaluate:
        ...
        validate(val_loader, train_loader, ensemble_models, criterion_ce, args)
        return


def validate(val_loader, train_loader, ensemble_models, criterion, args):
    ...
    ensemble_models = [model.eval() for model in ensemble_models]
