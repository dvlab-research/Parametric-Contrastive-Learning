...
parser.add_argument('--smooth', default=0.1, type=float, help='label smooth')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    criterion = PaCoLoss(alpha=args.alpha, temperature=args.moco_t, K=args.moco_k, smooth=args.smooth).cuda(args.gpu)
    ...
    if args.aug == 'randcls_sim':
       ...
       transform_train = moco.loader.TwoCropsTransformV2([transforms.Compose(augmentation_randncls), transforms.Compose(augmentation_randncls_stack)])
    ...
    if args.evaluate_r:
        evaluate_r(model, val_transform, args)
        return 

    if args.evaluate_s:
        evaluate_s(model, val_transform, args)
        return
...
def train(train_loader, model, criterion, optimizer, epoch, scaler, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        b = logits.size(0) // 2
        acc1, acc5 = accuracy(logits[:b], target, topk=(1, 5))
...
@torch.no_grad()
def evaluate_c(net, test_transform, args):
    # calculate
    corruption_accs = {}
...
@torch.no_grad()
def evaluate_r(net, test_transform, args):
    # calculate
    print('imagenet-r')
...
@torch.no_grad()
def evaluate_s(net, test_transform, args):
    # calculate
    print('imagenet-s')
