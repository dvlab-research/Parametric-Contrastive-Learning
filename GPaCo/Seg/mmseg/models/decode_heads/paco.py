...
class PaCoLoss(nn.Module):
    ...
    def forward(self, features, labels=None, sup_logits=None, mask=None, epoch=None):
        ...
        anchor_dot_contrast = torch.cat((sup_logits, anchor_dot_contrast), dim=1)
        ...
        # add ground truth 
        one_hot_label = torch.nn.functional.one_hot(labels.view(-1,), num_classes=self.num_classes+1)[:,:-1].to(torch.float32)
        ...
        loss = (one_hot_label.sum(1) * loss).sum() / (one_hot_label.sum() + 1e-12)
        return loss 
